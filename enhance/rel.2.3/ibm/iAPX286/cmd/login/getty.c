...
/*									*/
/*	Modifications:							*/
/*	M000:	uport!rex	Thu Jan 22 17:59:14 PST 1987		*/
/*		added login checking via a kernel table			*/
/*	M001:	uport!rex	Sun Feb  1 16:21:31 PST 1987		*/
/*		added checking with "init" that this "getty" is correct */
/*	M002:	uport!rex	Tue Mar 10 22:08:54 PST 1987		*/
/*		added ability to disable login checking			*/
/*									*/


#include	<stdio.h>
#include	<fcntl.h>	/* M000 */
#include	<sys/types.h>
#include	<termio.h>
#include	<signal.h>
#include	<sys/stat.h>
...
#define		QUIT		('\\'&037)	/* ^\ */
#define		ERASE		'\b'	/* normally #, but not for S5 */
#define		BACKSPACE	'\b'
#define		KILL		'@'
...
char	*checkgdfile;		/* Name of gettydefs file during
				   check mode. */
short	dflag = 0;		/* M000: debug flag set on command line */
...
char **argv;
{
	...
	static char loginchk = 1;    /* M002: 0 if not checking login table */

	signal(SIGINT,SIG_IGN);
	signal(SIGQUIT,SIG_DFL);

    /*
     *  M001:	Now we must let "init" know that this "getty" is the
     *		the correct version for limited login.  This is done
     *		by adding 0 to the kernel table,  0 meaning that the
     *		entry  is  free,   which also clears a kernel getty-
     *		check flag.
     */
	if (chklogcnt(0)) {
		fprintf(stderr, "Kernel version older than 1.3.8.\n");
		loginchk = 0;	/* M002: disable login checking */
	}
...
/* Get the user's typed response and respond appropriately. */
		...
		case GOODNAME:
			if (timeout) alarm(0);
	    /*
	     *  M000: Before any more processing, check to see if the maximum
	     *        login count has been reached if login checking has not
	     *	      been disabled.
	     */
			if (loginchk && chklogcnt(getpid())) {	/* M002 */
				fprintf(stderr,
				    "Login limit. Try again later.\n");
				sleep(20);
				exit(1);
			}
 ...
 *  looks to see if there is a free entry in a kernel login table.
 *  If there is, the "pid" is entered there, thus incrementing the current
 *  login count.
 */
chklogcnt(pid)
	int pid;
{
	...
	struct stat statb;
	if (pid) 	/* M001 */
	{		/* skip the console check for pid == 0 */
		if (fstat(fileno(stderr),&statb)) {
			fprintf(stderr, "Cannot stat stderr\n");
			return(1);
		}
		if ((statb.st_rdev & 0xff00) == 0)	/* console */
			if (!dflag) return(0);
	}
	...
	arg = pid;
	if (ioctl(mfd, IOCIOP_LOGADD, &arg) == -1) {
		if (pid)
			fprintf(stderr, "LL: LOGADD failed: %d\n", errno);
		close(mfd);
		return(1);
	}
	close(mfd);
	...
	{
#ifdef	ERRDEBUG
		gdebug("No empty login entries for %d.\n", pid);
#endif
		return(1);
	}
	return(0);
