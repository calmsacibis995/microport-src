...
/* iAPX286 @(#)conn.c	1.3 */
/* @(#)conn.c	1.13 */
/* M000 lance norskog, microport systems 4-14-86
 * 	modication made to fix problem in login scripts.
 *	Code had improper syntax for stripping parity bits from response data.
 *	For good measure, reparenthesized all ?: expressions.
 */
#define CONN
#include "uucp.h"
#include <signal.h>
...
fnd:
	...
	alarm((timelim < 30) ? 30 : timelim); /* M000 */
	if ((pid = fork()) == 0) {
		sleep(2);
		fclose(stdin);
		fclose(stdout);
...
register struct dlines *p, *q;
{
	...
	a = (p->d_type&D_ACU)?0:1; /* M000 */
	b = (q->d_type&D_ACU)?0:1; /* M000 */
	if(a == b)
		return(0);
	if(a < b)
		return(1);
...
char *str;
{
	...
	*rp = 0;
	...
	while (notin(str, rdvec)) {
		...
		DEBUG(4,"%c",((nextch&0177)>=040)?(nextch&0177):'_'); /* M000 */
		if ((*rp = (nextch & 0177)) != '\0') /* M000 */
			rp++;
		if (rp >= rdvec + MR){
			alarm(0);
			return(FAIL);
