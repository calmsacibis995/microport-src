static char *uportid = "@(#)slp.c	Microport Rev Id 2.2  4/28/87";
 ...
 *	1) Changed proc 0 to unmount all filesystems and reboot on 
 *		dieflag (set by ctrl-alt-del).  It calls
 *		kadmin() multiple times, but subsequent ones are ignored.
 *	M001:	uport!rex	Sun Feb  1 16:12:37 PST 1987
 *		Added a check that the correct versions of "init"
 *		is installed, if not, something terrible should happen.
 *	M002:	uport!rex	4/29/87
 *		made sure the new process didn't think it has a valid
 *		saved state for the floating point unit
 */
...
#ifdef ATMERGE
/* p_swaddr != 0 if memshuffle() has called xswap() to
* swap this process out to make room for the dos proc; don't swap in
*/
	    (rp->p_swaddr == 0) &&
#endif /* ATMERGE */
	    rp->p_time > outage) {
...
loop:
		if (dieflag) kadmin(A_SHUTDOWN, AD_BOOT, 1);
        spl7();
        runrun = 0;
        pp = NULL;
        q = NULL;
...
#ifdef ATMERGE
		if (p == dosprocp) {
			protectdos(0);   /* allow access to lower 1MB */
		} else {
			protectdos(1);	 /* disallow access to lower 1MB */
		}
#endif /* ATMERGE */
		dispatch(p);
...
newproc(i)
{
	...
	static mpid;
	register s;
	static	char	initnok = 1;	/* M001: 0 = init is ok	*/
	extern	char	initchk;	/* M001: cf/lomem.c	*/

	/*
	 *  M001:
	 *  This is a check that the limited login routines are in
	 *  place.  The correct "init" will clear this flag and the
	 *  sleep() should never happen.
	 *  I would rather signal "init" to single user mode than put
	 *  fork() calls to sleep, but this was a more immediate solution.
	 *  The print should help determine that a system is not booting
	 *  because the incorrect version of "/etc/init" is installed.
	 */
	if (initnok && i) {
		while (initchk) {
			printf("LL");
			sleep((caddr_t) &initchk, PZERO+1);
		}
		initnok = 0;	/* have passed init check, so don't */
	}			/* check it again */
	/* M001 */
...
retry:
	...
	u.u_fpvalid = 0;		/* M002: make sure the process doesn't
					   think it has a valid saved state */
	/*
