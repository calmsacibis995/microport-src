static char *uportid = "@(#)msg.c	Microport Rev Id 1.3.3  6/18/86";
...
**  Upgrade to the AT:
**  	changed msginit() to use valloc instead of malloc
** M000: Fixed msginit() again.  This time it just uses the address
**	 returned from valloc() and doesn't try to stuff the msg
**	 selector with it.
**	 Daniel M. Frank, Microport Systems.
*/
...
#endif
#ifdef iAPX286
	bs = (uint)btoc((long)msginfo.msgseg * msginfo.msgssz);
	if((long)msginfo.msgseg * msginfo.msgssz >= ctob((long)stoc(1)) ||
		(msg = valloc(ctob(bs), 0)) == NULL) {
#endif
