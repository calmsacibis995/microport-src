static char *uportid = "@(#)tty.c	Microport Rev Id 1.3.5  6/26/86";
 ...
 * M001 Add CLOCAL to default tty modes (don't require carrier)
 */
#include "sys/param.h"
#include "sys/types.h"
#include "sys/systm.h"
...
/* null clist header */
struct clist ttnulq = { 0, (struct cblock *) 0, (struct cblock *) 0 };
...
union ioctl_arg	arg;
{
	...
	case TCSETA:
		...
		if (tp->t_line != cb.c_line) {
			...
			/* (caddr_t) substituting for (union ioctl_arg) */
			(*linesw[tp->t_line].l_ioctl)(tp, LDCHG, (caddr_t)flag, mode);
		}
		return(1);
	...
	case TCFLSH:
		...
		case 1:
		case 2:
			ttyflush(tp, (arg.iarg - FOPEN)&(FREAD|FWRITE));
			break;
...
/* conversion aide only */
	...
	case TIOCGETP:
		...
		if (tp->t_oflag&BSDLY)
			flag |= O_BSDELAY;
		tb.sg_flags = flag;
		if (copyout(&tb, arg.sparg, sizeof(tb)))
			u.u_error = EFAULT;
		break;
...
register struct tty *tp;
{
	...
	tp->t_cflag = SSPEED|CS8|CREAD|HUPCL|CLOCAL;	/* M001 */
	tp->t_lflag = ECHO|ISIG|ICANON;
