static char *uportid = "@(#)sxt.c	Microport Rev Id 1.3.3  6/18/86";
...
union ioctl_arg arg;
{
	...
	case SXTIOCLINK:
		if(arg.iarg > MAXPCHAN || arg.iarg < 1) {
			u.u_error = EINVAL;
			break;
		}
		...
		tp += minor(UNMODEM(u.u_ttyd));	/* Real 'tty' line */
	...
	case SXTIOCSWTCH:
		...
		if (tracing)
			printf("!sxtioctl: switch arg=%d, control=%d\n",
				arg.iarg,lp->controllingtty);
#endif
		if ( lp->controllingtty != arg.iarg){
				
			lp->controllingtty = arg.iarg;
			if (arg.iarg != 0){
				/*  download valid portions of tty struct*/
				tp = lp->line;
				vtp = &lp->chans[arg.iarg].tty;
				/*  change flags */
		...
		splx(sps);
		wakeup( (caddr_t) &lp->chans[arg.iarg]);
		break;
	...
	case SXTIOCWF:
		...
		sps = spl5();
		while (lp->controllingtty != arg.iarg)
			sleep( (caddr_t) &lp->chans[arg.iarg], TTOPRI);
		splx(sps);
	...
	case SXTIOCUBLK:
		...
		if (!(1<<arg.iarg & lp->open) || (arg.iarg == 0)) {
			u.u_error = EINVAL;
			break;
		}
		vtp = &lp->chans[arg.iarg].tty;
	...
	case SXTIOCSTAT:
		...
		if (copyout(&sb, arg.sparg, sizeof sb)){
			u.u_error = EFAULT;
			break;
		}
		break;
	...
	case TIOCSETP:
		...
		if (CHAN(dev) == lp->controllingtty){
			...
			if (copyin(arg.sparg, &tb, sizeof(tb))) {
				u.u_error = EFAULT;
				break;
			}
			tp->t_iflag = 0;
	...
	case TCSETAF:
		...
		if (CHAN(dev) == lp->controllingtty){
			...
			if (copyin(arg.stparg, &cb, sizeof cb)){
				u.u_error = EFAULT;
				break;
			}
			c_line = cb.c_line;
			cb.c_line = lp->line->t_line;
			if (copyout( &cb, arg.stparg, sizeof cb)){
				u.u_error = EFAULT;
				break;
			}
	...
	case TCSBRK:
		ttiocom(&lp->chans[CHAN(dev)].tty, cmd, arg, mode);
		if (CHAN(dev) == lp->controllingtty && arg.iarg == 0)
...
sxtinit()
{
	...
	if((sxtbuf = kseg(RW, btop(sxt_cnt*LINKSIZE))) == NULL)
		printf("sxt cannot allocate link buffers\n");
}
