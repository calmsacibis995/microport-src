...
int idx;			/* block index (for debugging) */
...
rmlbls() { /* remove unreferenced labels */
	...
	/* if first time thru' scan the source and change any labels 
	   referenced from data section into hard labels	*/

	if( !npass )
	for (ALLN(p))
		if (islabel(p) && FINDLBL(p->ops[0]) != NULL)
			p->op = HLABEL ; /* HARDLABEL */
...
bldgr(opt) boolean opt; { /* build flow graph of procedure */
	...
	if( ( (long)idx * (long)sizeof(BLOCK) ) >= 0x10000L)
		fatal( "Out of space\n",(char *)NULL);
#endif
...
comtail() { /* merge common tails from code blocks */
	...
	do {
		...
		for (ALLB(bi, b0.next)) { /* compute a key for each block */
			...
			/* try to make these reasonably unique */
			bi->marked |= (bi0->nextl - b0.next) << 5;
		}
		for (ALLB(bi, b0.next)) {
			int cond_br;
...
/* clear chunk of memory - memset() takes an int count, we   */
/*			   want to clear with unsigned count */
clear(ptr, num) char *ptr; unsigned num; {

	if (num > MAXINT) {
		memset(ptr, 0, MAXINT);
		num -= MAXINT; ptr += MAXINT;
	}
	memset(ptr, 0, num);
}
...
ldanal() { /* perform live-dead register analysis */
	...
	if( ( (long)(idx+1) * (long)sizeof(struct ldinfo) ) >= 0x10000L)
		fatal( "Out of space\n",(char *)NULL);
#endif
...
#if iAPX286
	/*
	 *	p -= H_INCR may wrap round to top of segment and original test 
	 *	not sufficient to detect this
	 */
		if ( p < Lbltbl+H_INCR )
			 p += (N_LBLS-H_INCR) ;
		else
