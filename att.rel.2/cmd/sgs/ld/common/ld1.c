	...
	unsigned short	ftype;
{
	...
	else if( TYPE(ldptr) == USH_ARTYPE )
	{
		...
		if (FREAD(arsymtab,sizeof(longchar),(int)sym_count.l,ldptr)
			!= sym_count.l)
		...
		if (FREAD(arsymtab,sizeof(struct ar_sym),(int)sym_count,ldptr)
			!= sym_count)
			lderror(2,0,NULL,
...
#if iAPX286
				if (fread(&strtabptr[4], (int) (*sizeptr - 4), 1, fdes) != 1)
#else
		...
		while (fptr){	/* loop to process all library members used in
			...
			if (FSEEK(ldptr,0,BEGINNING) != OKFSEEK)
				lderror(2,0,NULL,"fail to seek to file header of %s",
					fptr->flname);
			if (FREAD((char *)(&(HEADER(ldptr))),FILHSZ,1,ldptr) != 1)
...
#if iAPX286
	short secnum = 1;	/* keep track of how may .bss sections we've made */
	long alignval;		/* address that the common symbol might appear at */
	long commalign();	/* routine that give what the alignment might be  */
#endif
...
#if iAPX286
			/* make sure that it will fit into this section */

			symsize = symp->sment.n_value;
			alignval = commalign( sptr->ishdr.s_size, symsize );

			if ( (alignval + symsize) > MAXSCNSZ ) {
				if ( model == M_SMALL ) 
					lderror(1,0,NULL, "Size of output section would cause segment overflow in section .bss\n\tRecompile with '-Ml' option");
				/* 
				 * first, round up old section size, add it
				 * to the list of insections for this file,
				 * and then create a new .bss section
				 */
				sptr->ishdr.s_size += 03L;
				sptr->ishdr.s_size &= (~03L);
				sptr = (INSECT *) mycalloc( sizeof(INSECT) );
				...
				sptr->ishdr.s_size = alignval;
			}
			symp->smscnptr = sptr;
			symp->sment.n_scnum = secnum;
#else
...
#if iAPX286
long
commalign(hdrsize,symsize)
long hdrsize, symsize;
{
	if (symsize == 2) {
		hdrsize += 1;
		hdrsize &= (~1);
 ...
 */
	...
	if (FREAD((char *)(&(HEADER(fdes))),FILHSZ,1,fdes) != 1)
		return(FAILURE);
