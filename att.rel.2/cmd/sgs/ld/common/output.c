 ...
 */
	...
	for( osp = (OUTSECT *) dsectlst.head; osp; osp = osp->osdsnext ) {
		/*
		 * Special processing for COPY, OVERLAY and INFO sections:
		 *  1) remove DSECT (signifying no alloc) flag
		 *  2) leave section size intact
		 */

		if ( osp->oshdr.s_flags & (STYP_COPY | STYP_INFO | STYP_OVER)) {
...
#else
	wrtinfl(fdes, sdes, rdes, (char *)buf);
#endif
			...
			while( indx < osp->oshdr.s_size ) {
				...
				if( disp > indx ) {	/* fill needed */
					...
					for( ; indx < disp-1; indx += sizeof(short) )
						fwrite((char *)&fill, sizeof(short), 1, fdes);
					if( indx < disp ) {
						fwrite((char *)&fill, sizeof(char), 1, fdes);
						indx++;
						}
					}
				if( isp != NULL ) {
 ...
 */
	if ( last_string_ptr )
#if iAPX286
		fwrite( last_string_ptr, (int) string_buf_length, 1, strdes );
#else
 ...
 */
	...
	outflhdr.f_timdat = (long) time((long *)0);
	outflhdr.f_opthdr = hflag;
	fseek(fdes, 0L, 0);		/* seek to beginning of file */
	fwrite((char *)&outflhdr, FILHSZ, 1, fdes);
 ...
 */
	...
	if( fill > 0 ) {
		...
		for( ; fill > 0; fill -= BUFSIZ ) 
			fwrite((char *)buf, min(fill, BUFSIZ), 1, fdes);
		}
...
if (max_section > maxint && (sect_buffer = malloc((unsigned)maxint))) {
	buffer_size = maxint;
} else
#endif
if (sect_buffer = malloc((unsigned)max_section ))
 ...
 */
	...
	for( isp = infl->flishead; isp; isp = isp->isnext ) {
		...
		fseek(fdes, offset, 0);
		if ((isp->ishdr.s_nreloc == 0)
			    || (osp->oshdr.s_flags & STYP_INFO)
			    || (osp->oshdr.s_flags & STYP_COPY))
			copy_section( ifd, infl, isp, osp, fdes, sect_buffer,
				buffer_size );
		else
		{
			if( rflag )
		...
		if( !sflag  && !xflag && isp->ishdr.s_nlnno > 0)  {
			...
			for( ndx = 0; ndx < isp->ishdr.s_nlnno; ndx++ ) {
				...
				if( lentry->l_lnno == 0 ) {
					...
					fseek(sdes, offset, 0);
					fwrite((char *)&lnptr, sizeof(long), 1, sdes);
					}
				else
 ...
 */
	...
	newscnum = (int *) myalloc((int)( i*sizeof(int) + 2*(i*sizeof(long))));
...
#if iAPX286
	/*
	 * if the ts or ds flag is on then it is assumed that
	 * we're link editing kernel code.  Since the boot code
	 * requires actual numbers of bytes, we just take the
	 * size that is in the section header.  If the Model
	 * type is LARGE and the ts and ds flags are NOT specified,
	 * then the sizes have to be in "click" sizes, which is
	 * why the headers are aligned.  This is what exec wants
	 * to see.
	 *
	 * For the small model, we just want the size given in
	 * the section header, with the stacksize being added to the data
	 * section size because the stack precedes the data and bss in
	 * consecutive memory (where stack + data + bss <= 64K bytes).
	 */

	/*
	 * initialize to 0 since this is an automatic structure and
	 * could have any garbage in it.
	 */
#define PAGSIZE	512
#define alignit(x)	( ((x) + PAGSIZE - 1 ) & ~(PAGSIZE - 1) )
	aout.tsize = 0;
	aout.dsize = 0;
	aout.bsize = 0;
#endif
	...
	for( osptr = (OUTSECT *) outsclst.head; osptr; osptr=osptr->osnext ) {
		if( equal(osptr->oshdr.s_name, _TEXT, 8) ) {
			...
			if ( model == M_SMALL || Kflag ) 
				aout.tsize += osptr->oshdr.s_size;
			else
				aout.tsize += alignit(osptr->oshdr.s_size);
			...
			if ( model == M_SMALL || Kflag ) 
				...
				aout.dsize += alignit(osptr->oshdr.s_size);
			...
			if ( model == M_SMALL || Kflag ) 
				...
				aout.bsize += alignit(osptr->oshdr.s_size);
...
#if iAPX286
		/* remember to add in the stack size in the small model! */
	if( model == M_SMALL )
		aout.dsize += kflag;
#endif
	if( found != 3 )
...
#if iAPX286
	count = ( (long)( (long)( psize() - sizeof(long) ) / PESIZE));
#else
	count = ( (long)((psize() - sizeof(long)) / PESIZE));
#endif
	fwrite((char *)&count, sizeof(long), 1, fdes);
...
long	cursym;
{
	...
	fwrite((char *)&cursym, sizeof(long), 1, symdes);
	fseek((char *)symdes, here-there-sizeof(long), 1);
	return(1);
}
...
int strlength;
{
	...
	{
#if iAPX286
		if (last_string_ptr && fwrite(last_string_ptr, (int) string_buf_length, 1, strdes ) != 1)
#else
