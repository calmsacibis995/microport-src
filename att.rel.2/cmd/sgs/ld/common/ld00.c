 ...
 */
...
#if iAPX286
	static char libname[MAXDLEN+22];	/* -lx file name */
	char	*modeldir;
#else
...
#if iAPX286 
	/* say what directory in the library directory to search */

	if (model == (short) (M_LARGE)) {
		modeldir = "/large";
	} else if (model == (short) (M_HUGE)) {
		modeldir = "/huge";
	} else {
		modeldir = "/small";
	}
	...
	{
#if iAPX286
		sprintf(libname, "%s%.6s%s%.8s%s", libdirs[dndx], modeldir,
				 "/lib", p, ".a");
#else
