...
#if iAPX286
			textbegin = (long) (hflag + FILHSZ + SCNHSZ * numoutsc + memorg);
#else
 ...
 */
	...
	do {
		...
		case AIADFILE:
			...
			if( ! dflt )
#if iAPX286
				a2ptr->adfile.aifilnam = (char *) ((long) a2ptr->adfile.aifilnam + (long) strbase);
#else
		...
		case AIADDSCN:
			...
			if( ! dflt )
#if iAPX286
				a2ptr->addscn.aiscfile = (char *) ((long) a2ptr->addscn.aiscfile + (long) strbase);
#else
...
#if iAPX286
	/*
	 * have to check to make sure that the section that we are
	 * about to build is containable in MAXSCNSZ.
	 */

	if ( ( (*poutptr)->oshdr.s_size + pin->ishdr.s_size ) > MAXSCNSZ ) {
		if (model == M_SMALL) {
			lderror(1,0,NULL, "Size of output section would cause segment overflow in section %s\n\tRecompile with '-Ml' option", (*poutptr)->oshdr.s_name);
		}
		/* 
		 * Have to see if this input section will fit into another
		 * output section of the same name.
		 */
		newpout = *poutptr;
		while ( (newpout = fndnxtoutsec( newpout )) && 
			( newpout->oshdr.s_size + pin->ishdr.s_size > MAXSCNSZ )
			);
		/* At this point, we either have an output section that this
		 * input will fit into, or we need to create a new output
		 * section.
		 */
		if ( newpout == NULL ) {
			/* have to create a new section */
			newpout = (OUTSECT *) mycalloc(sizeof(OUTSECT));
			listadd(l_OS, &outsclst, newpout);
			numoutsc++;
			pout = *poutptr;
			copy(newpout->oshdr.s_name, pout->oshdr.s_name, 8);
				/*
				 * new section's address starts on next SEGMENT
				 * boundary. (have to make sure not to increment
				 * those 3 protection bits).
				 */
			newpout->oshdr.s_paddr = pout->oshdr.s_paddr;
			newpout->oshdr.s_vaddr = pout->oshdr.s_vaddr;
			newpout->oshdr.s_flags = pout->oshdr.s_flags;
			newpout->osfill = pout->osfill;
			newpout->osnflnm = pout->osnflnm;
			newpout->osnlnno = pout->osnlnno;
			newpout->osflags = pout->osflags;
		}
		pout = newpout; /* now go through the rest of the code using
				 * the name of the new output section.
				 */
		*poutptr = newpout; /* make sure that the caller used the
				     * correct output section in further 
				     * manipulations.
				     */
...
#if iAPX286
		if (model == M_SMALL) {
			lderror(1,0,NULL, "Segment overflow in section %s\n\tRecompile with '-Ml' option", osp->oshdr.s_name);
		}
#endif
...
{
	register OUTSECT *p, *q;
	char *nam = outptr->oshdr.s_name;

	for( p = outptr->osnext; p; p=p->osnext ) {
		if (p->oshdr.s_flags & STYP_GROUP)
			for( q = (OUTSECT *) p->osinclhd; q; q=q->osnext ) {
				if (equal(nam,q->oshdr.s_name,8))
					return(q);
