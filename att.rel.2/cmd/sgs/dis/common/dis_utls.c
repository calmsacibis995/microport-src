...
LDFILE	*ptr;
{
	...
	if ( FREAD((char *)&cur2bytes,sizeof(cur2bytes),1,ptr) == NULL){
		fprintf(stderr,"\n%s%s: premature EOF\n",SGS,TOOL);
		exit(4);
	}
	loc += 2;
...
LDFILE	*ptr;
{
	...
	if ( FREAD((char *)&cur4bytes,sizeof(cur4bytes),1,ptr) == NULL){
		fprintf(stderr,"\n%s%s: premature EOF\n",SGS,TOOL);
		exit(4);
	}
...
symfind:
	...
	if ((filhdr.f_nsyms != 0L) && (symb != NULL) && (ldtbseek(symb) != FAILURE)) {
		...
		ldtbread(symb, (long) firstfn, septr);
