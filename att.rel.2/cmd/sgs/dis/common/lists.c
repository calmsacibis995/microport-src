...
build_sections()
{
	...
	if (nsecs >= 0)
	{
		for ( i = 0; i <= nsecs; i++ )
		{
			FSEEK( f_ptr,(long) ( FILHSZ + filhdr.f_opthdr), BEGINNING );
		...
		for ( sect = 1; sect <= filhdr.f_nscns; sect++ )
		{
			if (ldshread (f_ptr,(unsigned short) sect, &scnhdr ) == FAILURE)
...
/* we need this with  AR16WR (S5) #if !AR16WR || OLIST  */
build_funcs()
{
	extern	char	*calloc();
	...
	for ( i = 0; i < filhdr.f_nsyms; i++)
	{
		...
		if (ISFCN( symbol.n_type ) || ((symbol.n_sclass==C_EXT ||
		   ...
		   (sectp != NULL && strncmp(sectp->shdr.s_name,".text",8) == 0)
		&& (strncmp(symbol.n_name,".text",8) != 0)))
		{
			...
			if ((func->funcnm = calloc(1, (unsigned)(strlen( func_name ) + 1))) == NULL)
				fatal( "memory allocation failure" );
