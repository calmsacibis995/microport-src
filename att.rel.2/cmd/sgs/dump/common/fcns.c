...

{
	...
	for (i = HEADER(ldptr).f_opthdr / sizeof(short); i > 0; --i, ++count) {
	    if (FREAD((char *)&buffer, (int) sizeof(short), 1, ldptr) != 1) {
	...
	if ((HEADER(ldptr).f_opthdr & 0x1) == 1) {
	    if (FREAD((char *)&buffer, (int) sizeof(char), 1, ldptr) != 1) {
		fprintf(stderr, "%sdump:  cannot FREAD %s\n", SGS, filename);
	    }
	    printf("%.4hx", buffer);
	}
...

{
	...
	for (	; first <= last;  ++first)
	{
		...
		if (secthead.s_nreloc != 0)
		{
			...
			{
				if (FREAD((char *)&relent, RELSZ, 1, ldptr) != 1)
				{
					fprintf(stderr, "%sdump:  cannot FREAD %s\n", SGS, filename);
					return(FAILURE);
				}
...

{
	...
	for (	; first <= last;  ++first)
	{
		...
		{
			if (FREAD((char *)&linent, LINESZ, 1, ldptr) != 1)
			{
				fprintf(stderr, "%sdump:  cannot FREAD %s\n", SGS, filename);
				return(FAILURE);
			}
...

{
	...
	if ( arbuf.ar_name[0] == '/' )
	{
		...
		if ((offsets = calloc((unsigned) num_syms, 
                     (unsigned) (sizeof(long) * sizeof(char)) )) == NULL)
		{
			fprintf( stderr, "cannot allocate space for array\n");
			return( FAILURE );
		}
		if ( FREAD( offsets, sizeof(long) * sizeof(char), (int) num_syms,
			ldptr) != num_syms)
		{
