...

{
	...
	if (buildtmp(filename, &filehead) == SUCCESS)
	{
		...
		if (copytmp(filename, &filehead) == SUCCESS)
		{
			if (arhead.ar_size & 1) {
				if (putc('\n', stripout) == EOF) {
					error((char *)strp2,"cannot write to tmp file",5);
					exit(FATAL);
				}
			}
		 	if (xmore == SUCCESS ) 
...
/*	LOCAL VARIABLES USED */
	...
	*++p = '/';
	...
	strncpy( arbuf.ar_fmag, ARFMAG,(int)(sizeof( arbuf.ar_fmag )) );
	if (fwrite((char *) &arbuf,(int)( sizeof( struct ar_hdr )), 1, stripout ) != 1)
#else
