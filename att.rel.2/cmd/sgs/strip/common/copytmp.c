...

{
    ...
    if (fwrite((char *)filehead,(int) FILHSZ, 1, stripout) != 1)
	{
    ...
    while ((nitems = fread(buffer, (int)sizeof (char), BUFSIZ, strp1)) != 0) {
	if (fwrite(buffer,(int)sizeof (char), nitems, stripout) != nitems) {
	    error(filename, "cannot recreate stripped file<tmp1>", 5);
	    resetsig( );
	    return(FAILURE);
	}
    }
    while ((nitems = fread(buffer,(int)sizeof (char), BUFSIZ, strp2)) != 0) {
	if (fwrite(buffer,(int)sizeof (char), nitems, stripout) != nitems) {
