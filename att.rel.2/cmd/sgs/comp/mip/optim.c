...
optim(p) register NODE *p; {
	...
	case MINUS:
		...
		if( LCON(p) && conval(p->in.left, MINUS, p->in.right) ) {
		...
		*/
		...
		if(o == PLUS && LO(p) == MINUS && RCON(p) && RCON(p->in.left) &&
		  conval(p->in.right, MINUS, p->in.left->in.right)){
			sp = p->in.left;
			if( ISPTR(sp->in.right->tn.type) ) {
				p->in.right->in.type = p->in.type;
				sp->in.left = makety(sp->in.left, INT, 0, INT);
			}
			zapleft:
			RO(p->in.left) = FREE;
			LO(p) = FREE;
