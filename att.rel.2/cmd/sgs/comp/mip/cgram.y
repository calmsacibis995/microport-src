...
declarator:	   fdeclarator
		...
		|  nfdeclarator COLON con_e
			...
			={  if( !(instruct&INSTRUCT) ) UERROR( MESSAGE( 38 ) );
			    ...
			    defid( tymerge($<nodep>0,$1), FIELD | (int)$3 );
			    $$ = NIL;
			    }
		|  COLON con_e
			%prec CM
			/* "field outside of structure" */
			={  if( !(instruct&INSTRUCT) ) UERROR( MESSAGE( 38 ) );
			    falloc( stab, (int)$2, -1, $<nodep>0 );  /* alignment or hole */
			    $$ = NIL;
			    }
		|  error
			={  $$ = NIL; }
...
nfdeclarator:	   MUL nfdeclarator		
		...
		|  nfdeclarator LB con_e RB	
			={  bary:
				...
				if( $3 <= 0 ) WERROR( MESSAGE( 119 ) );
				$$ = bdty( LB, $1, $3 );  }
		|  NAME  		
			{
			$$ = bdty( NAME, NIL, (long)$1 );
				/*CXREF def($1, lineno); */
			}
		|   LP  nfdeclarator  RP 		
			={ $$=$2; }
...
fdeclarator:	   MUL fdeclarator
		...
		|  name_lp
			...
			={
				...
				$$ = bdty( UNARY CALL, bdty(NAME,NIL,(long)$1), 0L );
				stwart = 0;
				}
...
begin:		  LC
			={  setdim();
			    ...
			    ++blevel;
			    if( psavbc > &asavbc[BCSZ-3] ) cerror( "nesting too deep" );
			    *psavbc++ = regvar;
			    STKOFFSZ( *psavbc ) = autooff;
			    psavbc += 2;
			    }
		;
...
null_decl:	   /* empty */
			={ $$ = bdty( NAME, NIL, -1L ); }
		|  LP RP
			={ $$ = bdty( UNARY CALL, bdty(NAME,NIL,-1L), 0L ); }
		|  LP null_decl RP LP RP
			={  $$ = bdty( UNARY CALL, $2, 0L ); }
		|  MUL null_decl
			={  goto umul; }
		|  null_decl LB RB
			={  goto uary; }
