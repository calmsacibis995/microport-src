...
# endif
	...
	case REG:
		...
		*/
		...
		mask = rstatus[p->tn.rval] & (STAREG|SAREG|STBREG|SBREG);
		if((mask & (STAREG|STBREG)) && (busy[p->tn.rval] > 1))
			mask &= ~(STAREG|STBREG);
		return( shape & mask );
...
# ifndef OLDZZZ
			...
			case 'Z':
				zzzcode( (c=='Z')?p:getlr( p, c ), *++cp, cookie );
				break;
...
# endif
		...
		case 'D':  /* this line deleted if cdebug not set */
			c = *++cp;
			if( !cdebug )
				while( *++cp != c )
					{}
		...
		case 'R': /* reserve requested register */
			n = getlr( p, *++cp );
			r = atoi( ++cp );
			if(( n->in.op == REG ) && ( n->tn.rval != r )) {
				if( busy[r] != 0 )
					cerror( "reserved register %s not available\n", rnames[r] );
				n->tn.rval = r;
			}
			while( (*cp >= '0') && (*cp <= '9') )
				cp++;
