...
eprint( p, down, a, b ) NODE *p; int *a, *b; {
	...
	*a = *b = down+1;
	...
	printf( "%lo) %s", (OFFSZ)p, opst[p->in.op] );
...
# ifdef SSPLIT
	case SSPLIT:
	case UNARY SSPLIT:
		printf(" target %s", rnames[p->in.segment & ~MUSTDO]);
		if(p->in.segment & MUSTDO)
			printf(" MUSTDO");
		printf(", index %d", p->tn.rval);
		break;
# endif
	...
	tprint( p->in.type );
	printf( ", " );
	if( p->in.rall & NEEDREGS ) printf( "NEEDREGS" );
	else if( p->in.rall == NOPREF ) printf( "NOPREF" );
	else {
...
# else
		...
		p->in.right->in.segment = p->in.segment;
		...
		if( o != 0 ){
			shp = talloc();
			shp->in.op = RS;
			shp->in.segment = p->in.segment;
			shp->in.rall = NOPREF;
			shp->in.type = ty;
			shp->in.left = p->in.left;
			shp->in.right = talloc();
			shp->in.right->in.op = ICON;
			shp->in.right->in.segment = p->in.segment;
