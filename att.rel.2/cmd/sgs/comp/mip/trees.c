...
int bdebug = 0;
extern ddebug;
int fpe_flag = 0;  /* floating point error */
...
*/
		...
		l->fpn.dval = dcon;
		if( fpe_flag ) {
		    l->fpn.dval = l->fpn.dval < 0 ? -HUGE : HUGE;
		    fpe_flag = 0;
		}
		return(l);
		}
	...
	if( actions & OTHER ){
		...
		case STREF:
			...
			if( stab[i].sflags & SNONUNIQ &&
				ISPTR(l->in.type) &&
				(BTYPE(l->in.type) == STRTY || BTYPE(l->in.type) == UNIONTY)
				&& (l->fn.csiz +1) >= 0 ){
...
conval( p, o, q ) register NODE *p, *q; {
	...
	if( SZLONG > SZINT ){
		if( p->in.op == ICON &&
		     ( p->in.type == ULONG || q->in.type == ULONG ) )
				p->in.type = q->in.type = ULONG;
		if( p->in.op == ICON &&
		     ( p->in.type == LONG || q->in.type == LONG ) )
	...
	case PLUS:
		p->tn.lval += val;
		/* iAPX286 contant type fix (4/15/85) - mw */
		if( p->tn.rval == NONAME ) {
			if ( q->tn.rval != NONAME ){
				p->tn.rval = q->tn.rval;
				p->in.type = q->in.type;
			}
			else if (ISPTR(q->in.type)) p->in.type = q->in.type;
		}
		break;
	case MINUS:
		p->tn.lval -= val;
...
/*FRIG*/                        p->tn.lval = (unsigned long)p->tn.lval / val;
		else p->tn.lval /= val;
		break;
	case MOD:
		/* "division by 0" */
		if( val == 0 ) UERROR( MESSAGE( 31 ) );
/*FRIG*/                else if( u ) p->tn.lval = (unsigned long)p->tn.lval % val;
		else p->tn.lval %= val;
		break;
	case AND:
		p->tn.lval &= val;
	...
	case RS:
		i = val;
/*FRIG*/                if( u ) p->tn.lval = (unsigned long)p->tn.lval >> i;
...
stref( p ) register NODE *p; {
	...
	if( dsc & FIELD ){
		...
		p->tn.rval = PKFIELD( dsc&FLDSIZ, (int)q->offset%align );
		}
...
convert( p, f )  register NODE *p; {
	...
	r = clocal(r);
	if( f == CVTL ) {
		if(p->in.right->in.type & LONGSIZE)
			r = makety( r, ULONG, r->fn.cdim, r->fn.csiz );
		p->in.left = r;
	} else {
		if(p->in.left->in.type & LONGSIZE)
			r = makety( r, ULONG, r->fn.cdim, r->fn.csiz );
		p->in.right = r;
	}
	return(p);
...
econvert( p ) register NODE *p; {
	...
	if( (ty=BTYPE(p->in.type)) == ENUMTY || ty == MOETY ) {
		...
		if( p->in.op == ICON && ty != LONG && ty != ULONG )
...
/* NOTE the is an addition which is NOT in the official pcc source
...
*/
	...
	else if( t1==LONG || t2==LONG || t1&LONGSIZE || t2&LONGSIZE) t = LONG;
	else t = INT;
...
block( o, l, r, t, d, s ) register NODE *l, *r; TWORD t; {
	...
	if(bdebug>1) printf( "block %lo -> %s %o %d %d %o %o\n",
			p, opst[p->in.op], p->in.type, p->fn.cdim,
			p->fn.csiz, p->tn.lval, p->tn.rval);
	return(p);
	}
...
doszof( p )  register NODE *p; {
	...
	p->fn.csiz = p->fn.type =
			(model> MDL_LARGE) ? ctype( ULONG ) : ctype( INT );
	return( p );
	}
