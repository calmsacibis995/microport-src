...
int MAXRVAR = 5;
int MINRVAR = 6;	/* no reg vars for now - woa */
...
# endif
	...
	if((ISPTR(type) || ISARY(type)) && farlist(p->sname))
		p->stype = type | LONGSIZE;
		else p->stype = type;
	p->sclass = class;
	p->slevel = blevel;
	p->offset = NOOFFSET;
...
ftnend(){ /* end of function */
	...
	paramno = 0;
	autooff = AUTOINIT;
	minrvar = regvar = MAXRVAR = (model < MDL_LARGE) ? 5 : 3;
	reached = 1;
	swx = 0;
	swp = swtab;
	locctr(DATA);
...
bstruct( idn, soru ){ /* begining of structure or union declaration */
	...
	psave( instruct );
	psave( curclass );
	STKOFFSZ( paramstk[ paramno ] ) = strucoff;
	paramno += 2;
	strucoff = 0;
	instruct = soru;
	q = block( FREE, NIL, NIL, 0, 0, 0 );
...
# endif
	...
	strucoff = STKOFFSZ( paramstk[ oparam+2 ] );
	paramno = oparam;
...
# endif
		...
		pstk->in_x =  t==STRTY ? (int)dimtab[s+1] : 0 ;
...
OFFSZ size; register alignment; register OFFSZ *poff; {
	...
	if( instruct != INSTRUCT ) {
		if( off + size > MAXAUTO )
			uerror("too many local variables");
	}
	else
		if( off + size > MAXSTRUCT )
			uerror("Structure too large");
	*poff = off+size;
	return( off );
	}
...
mknonuniq(idindex) int *idindex; {/* locate a symbol table entry for */
	...
	if ((new = freehead) == NULL) /* locate unused entry */
	   cerror("symbol table full");
	freehead = freehead->link;
	new->link = sp->link;
	sp->link = new;
	new->sflags = SNONUNIQ | SMOS;
#ifdef FLEXNAMES
	new->sname = stab [i].sname;
	*idindex = new - stab;
#ifndef BUG1
	if ( ddebug )
	{
...
#endif
	i = i%SYMTSZ;
	if ((next = hashhead [i]) != NULL)
	  {
/*	   prev = NULL;*/
	   do
             { /* look for name */
	        if ((next->sflags & (STAG|SMOS|SHIDDEN)) == s)
		  {
#ifdef FLEXNAMES
		   if ( next->sname == name )
#else
		   ...
		   for( j=0; j<NCHNAM;++j )
			...
			if( !*q++ ) break;
		     ...
		     {
/*
                      if ((prev != NULL) && (!(next->sflags & SHIDDEN))) 
		        {
                         prev->link = next->link;
                         next->link = hashhead [i];
                         hashhead [i]= next;
		        }
*/
		      return (next - stab);
...
nomatch:
 /*                  prev = next;*/
           ...
           if ((next = freehead) == NULL)
		cerror( "symbol table full" );
           freehead = freehead->link;
           next->link = hashhead [i];
           hashhead [i] = next;
       	   next->sflags = s;  /* set STAG, SMOS if needed, turn off all others */
#ifdef FLEXNAMES
	   next->sname = name;
#else
   	   p = next->sname;
...
clearst( lev ){ /* clear entries of internal scope  from the symbol table */
	register struct symtab *next, *prev;
	register int temp, i, j;
	struct symtab *hidden;
	int s;
	...
	for (i = 0; i < SYMTSZ; i++ )
	  {
	   if ((next = prev = hashhead [i]) != NULL)
              do
	        {
		 if (next->slevel > lev)  /* must clobber */
		   {
	    	    if (next->stype == UNDEF || (next->sclass == ULABEL && lev < 2 ))
		      {
			lineno = temp;
				/* "%.8s undefined" */
				/* "%s undefined" */
			UERROR( MESSAGE( 4 ), next->sname );
		      }
		    else aocode(next);
# ifndef BUG1
#ifdef FLEXNAMES
...
#endif
				next->sname,next-stab,next->sflags,next->slevel);
# endif
		    if (next->sflags & SHIDES)
		      {
		       s = next->sflags & (SMOS|STAG);
		       hidden = next->link;
		       while (hidden != NULL)
		         {	
		          if( (hidden->sflags&(SMOS|STAG)) == s )
			    {
#ifdef FLEXNAMES
			     if ( next->sname == hidden->sname )
#else
			     for( j =0; j<NCHNAM; ++j )
			        if( next->sname[j] != hidden->sname[j] ) break;
			     if( j == NCHNAM ) /* name is same */
#endif
				{
		       		hidden->sflags &= ~SHIDDEN;
				break;
				}
			    }
		          hidden = hidden->link;
...
# ifndef BUG1
		       if( ddebug )
		          printf( "%d uncovered from %d\n", hidden-stab,next-stab);
# endif
		   ...
		   if (prev == next)
		     {
		      hashhead [i] = next->link;
		      next->link = freehead;
		      freehead = next;
		      prev = next = hashhead [i];
		     }
		   else
		     {
		      prev->link = next->link;
...
hide( p ) register struct symtab *p; {
	...
	*q = *p;
	q->link = p->link;
        p->link = q;
	q->sflags |= SHIDDEN;
	p->sflags = (q->sflags&(SMOS|STAG)) | SHIDES;
