...
#if TARGET == VAX | TARGET == PDP11 | TARGET==IAPX286
...
#if TARGET == VAX | TARGET == X3B | TARGET == IAPX286
...
/* */
			...
			if (pt == TYREAL && lt == TYDREAL)
				break;
#if TARGET == IAPX286
			if ( (pt == TYLONG && ONEOF(lt, M(TYSHORT)|MSKADDR) ) ||
			     (lt == TYLONG && ONEOF(pt, M(TYSHORT)|MSKADDR) ) )
				break;
#endif
...
putx( call4(TYSUBR, "s_cat", lhs, cp, lp, mkconv(TYSHORT, ICON(n)) ) );
putcomma(ncomma, TYINT, NO);
}
