...
#if TARGET==PDP11 | TARGET==IAPX286
...
switch(opcode)
	...
	case OPSTAREQ:
		...
		if( ONEOF(ltype, MSKADDR|MSKINT) && ONEOF(rtype, MSKADDR|MSKINT)
			...
			break;
#endif
		if((ONEOF(ltype, MSKADDR|MSKINT)
			 && ONEOF(rtype, MSKADDR|MSKINT))
		    && (((typesize[ltype]==typesize[rtype] ))
		    || ((typesize[ltype]>=typesize[rtype] ) && ISCONST(rp)
		    && (modelflg > 0))))
			break;
	...
	case OPMINUS:
#if TARGET == IAPX286
		if (ltype == TYADDR && rtype == TYLONG) {
			p->rightp = fixtype( mkconv(TYSHORT, rp) );
			ptype = TYADDR;
			break;
		}
		if (ltype == TYLONG && rtype == TYADDR) {
			p->leftp = fixtype( mkconv(TYSHORT, lp) );
			ptype = TYADDR;
			break;
		}
#endif
 ...
 */
		...
		q = call2((ltype == TYREAL) ? TYDREAL : ltype,
			powint[ltype-TYLONG], lp, mkconv(TYINT,rp));
		if (ltype == TYREAL)
			q = mkconv(TYREAL, q);
		}
	}
