...
#if iAPX286
extern struct sdbsegdata *segdatap;
extern int *scnhdri;
#endif
...
initfp()
{
	...
	AUXENT *axp = &auxent[0];
	struct filet *filew;
	struct proct *procpbf,*procw,*procww;
	...
	soffset = filhdr.f_symptr;
#if iAPX286
	filep = files = badfile = ffreechain = fendchain = NULL;
	procp = procs = badproc = pfreechain = NULL;
	badfile = &badfilea;
	badfile->sfilename[0] = '\0';
	badproc = &badproca;
	badproc->pname = "";
	...
	for( ; ; soffset = noffset) {	/* stop when rdsym() returns ERROR */
	 	...
	 	*/
		/* note: iAPX286
		*  filet and proct entries are chained to-gether
		*  filet is two way chained 
		*  proct is single chained
		*  when space expires then the space is increased 
		*  filet by one click (512 bytes) and chained into 
		*        filet records on the free chain
		*  proct by two click (1024 bytes) and chained into
		*   	 proct records on its free chain
		*/
#if iAPX286
			if( ffreechain == NULL )
			{
				p = sbrk(ctob(FILEINCR)); /* get one click and carve */
				if ( p == (char *)-1 )
				{
					perror("sdb(sbrk)");
					exit(4);
				}
				ffreechain = (struct filet *)p;
				/* chain in the free entries */
				for (i=(ctob(FILEINCR)/sizeof files[0])-1;i;i--) 
				{
					filew = ffreechain;
					filew->fchain = ++ffreechain;
				}
				ffreechain->fchain = NULL;
				ffreechain = (struct filet *)p;
			}
			/* now take an entry off the free chain */
			if (files == NULL)
			{
				filep = files = ffreechain;
				filep->fbchain = NULL;
			}
			else
			{
				filep = fendchain->fchain = ffreechain;
				filep->fbchain = fendchain;
			}
			ffreechain = filep->fchain;
			filep->fchain = NULL;
			fendchain = filep;
			filep->sfilename[0] = '\0';
#else
		...
		if (ISFCN(type) || notstb)
		{
			...
			if (pfreechain == NULL)
			{
				if ((p = sbrk(ctob(PROCINCR))) == (char *)-1)
				{
					perror("sdb(sbrk)");
					exit(4);
				}
				pfreechain = (struct proct *)p;
				for(i=(ctob(PROCINCR)/sizeof procs[0])-1;i;i--)
				{
					procw = pfreechain;
					procw->pchain = ++pfreechain;
				}
				pfreechain->pchain = NULL;
				pfreechain = (struct proct *)p;
#else
			if (procp == badproc)
			{
...
#if iAPX286
			if((filew = filep->fbchain) != NULL)
			{
				if (filew->faddr == NEEDVAL)
				filew->faddr = procp->paddress;
			}
#else
...
#if iAPX286
			procp->sfptr = notstb ? badfile : filep;
#else
...
#if iAPX286
	procww = NULL;
	for(procw = procs;procw != NULL;procw = procw->pchain)
	{
		if(procs == procp)
		{
			break;
		}
		if( compar(procp,procw) < 0)
		{
		 	if(procww == NULL)
			{
				procs = procp;
...
/*   ?			if (!extstart)	extstart = soffset;*/
	...
	{
		fendchain->fchain = badfile;
	}
	badfile->fchain = NULL;
	badfile->fbchain = fendchain;
	if (procs == NULL)
	{
		procs = badproc;
	}
	...
	{
		for (procw=procs;procw->pchain != NULL;procw = procw->pchain);
		procw->pchain = badproc;
	}
	badproc->pchain = NULL;
#else
...
#endif
#if iAPX286
			for( ; (f!=NULL) && (f->lineflag); f=f->fbchain)
				;
			if( f == NULL) 
			{
				error ( "Bad file array" );
				f = (struct filet *)badproc;
			}
			return( f );
#else
			for( ; f->lineflag; f-- )
...
/*		if (filhdr.f_symptr < filestart && filestart < extstart)
...
/*		return(filhdr.f_symptr + axp->x_sym.x_tagndx * SYMESZ);
...
register long offset;
{
	...
	while( (offset = rdsym(offset)) != ERROR) {
		...
		if( eqpatu( pat, syp->n_nptr ) && syp->n_nptr[ 0 ] )
		{
			...
			if( sl_type == 0 &&
				((int)syp->n_scnum) <= 0 ||
				!(segdatap[scnhdri[syp->n_scnum-1]].segtype &
					 SEG_DATA))
#else
...
arystrdata()
{
	...
	if (ISTRTYP(i))
	{
		tagoff = rdsym(filhdr.f_symptr + axp->x_sym.x_tagndx * SYMESZ);
		if (tagoff <= 0) 	/* error */
		{
			sl_size = 0;
		}
...
#if iAPX286
		scalefctr = scalefctr*typetosize(type,sl_size);
#else
...
ADDR addr; {
	...
	lastproc = badproc;
#if iAPX286
	for (procp=procs; procp!=NULL; procp=procp->pchain) {
#else
	...
	if(lastproc != badproc && rdsym(lastproc->st_offset) != ERROR) {
		...
		if(debugflag)
#if iAPX286
			fprintf(FPRT2, "	: ladr=%#lx, st_off=%#lx;\n",
#else
...
register struct proct *procp;
{
	...
	if(debugflag > 0)
		fprintf(FPRT2, 
#if iAPX286
		    "adtostmt(addr=%#lx, procp=%#lx); pname=%s; n_value=%#x\n",
#else
...
#if iAPX286
	 *  On iAPX286,functions also start at first address in procedure
#endif
	 */
	...
	if( (offset = rdlntry(offset)) == ERROR || lnp->l_lnno != 0) {
		...
		if(debugflag)
#if iAPX286
			fprintf(FPRT2, "Cannot rdlntry(%#lx) for %s;\n",
#else
...
struct stmt stmt; char *filename; {
...
#if iAPX286
	for (pp=procs; pp!=NULL; pp=pp->pchain) {
#else
	for (pp=procs; pp->pname[0]; pp++) {
#endif
#if DEBUG > 1
		if(debugflag > 2) {
			fprintf(FPRT2,
				"stmttoproc(%d): pp->lineno=%d; lineno=%d;\n",
						ln, pp->lineno, ll);
#if iAPX286
			fprintf(FPRT2, "	pp=%#lx, pp->pname=(%#lx)%s\n",
						pp, pp->pname, pp->pname);
#else
	...
	if(debugflag)
		fprintf(FPRT2,
#if iAPX286
			"stmttoproc: pps->pname=%s; pps->st_offset=%#lx;\n",
#else
...
struct stmt stmt; long offset; char *file; {
...
#if iAPX286
		"\n	stmttoaddr(stmt=%d(%d),offset=%ld(%#lx),file=%s);\n",
#else
	...
	if(debugflag > 1)
#if iAPX286
		fprintf(FPRT2, "stmttoaddr: offl = %#lx; offn = %#lx;\n",
#else
...
#if DEBUG
#if iAPX286
	if (debug) fprintf(FPRT2, "getaddr() computed offset %#lx\n", offset);
#else
...
ADDR addr; {
	...
	if(debugflag)
#if iAPX286
		fprintf(FPRT2, "  : prevdiff=%#lx; sl_name=%s;\n",
#else
...
#if iAPX286
	register int i;
	register long prevdiff = 0x7fffffff,diff;
#else
...
sout:	if (amap.mp == 0) {
	...
	for (; *s; s++) {
		if (*s >= '0' && *s <= '9')
			...
			switch(level) 
			{
			case 0: /* segment recived */
				level++;
				inputseg = readint(&s);
				seg=((inputseg>>(amap.m->shift+16))&0xffff);
				amap.m->segdatap[seg].segvaddr =
					inputseg&amap.m->mask;
				break;
			case 1: /* size of the area */
				level++;
				amap.m->segdatap[seg].segvsize = readint(&s);
				break;
			case 2: /* position of the area */
				level++;
				amap.m->segdatap[seg].coreoff = readint(&s);
				break;
...
struct proct *procp;
{
	...
	register LINENO *lnp = &linent;
#if iAPX286
	int segsel;             /* segment selector from address */
#endif
	int n, nz = 0;
	...
	for(pp=procs;pp!=NULL;pp=pp->pchain)
	{
		n++;	/* number of the proc function assume proct order */
	}
#else
	n = procp - procs;	/* function number; assume proct order */
#endif
#if iAPX286
	offl = segdatap[SEGNO(procp->paddress)].scnptr->s_lnnoptr;
#else
	...
	if(debugflag)
#if iAPX286
		fprintf(FPRT2, "procs=%#lx; procp=%#lx; n=%d; offl=%#lx;\n",
#else
...
#if iAPX286
	shstrtab = (struct sh_name *)-1;
	areasize=((filhdr.f_nsyms*sizeof(struct sh_name))+3)&~3;
	if ( areasize < 64000L)
		shstrtab = (struct sh_name *)malloc((int)areasize);
	if (shstrtab == (struct sh_name *)(-1))
#else
...
#if iAPX286
	strtab = ( char *)-1;
	areasize = ((strtablen)+3)&~3;
	if (areasize < 64000L)
		strtab = malloc((int)areasize);
	if ( strtab == ( char * )(-1))
#else
