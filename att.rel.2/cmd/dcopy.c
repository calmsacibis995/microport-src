...
#define	NIBLOCKS	  20		/* Max. inode blocks read at once */
#define	MIBLOCKS	   6		/* Min. inode blocks to be read */
...
char **argv;
{
	...
	long	t;
#ifdef iAPX286
	long	iage;		/* holds age of file in days since 1970
				   multiplied by 60 which is too large for int*/
#else
	int	iage;
#endif
	int	j;
...
#ifdef iAPX286
/*
   The following lines of code to set up all[1].baseblock assumes that the file
   system is at least two cylinders long. (Alloc() also assumes this.)
   On iAPX286 neither of these assumptions is neccessarily valid, so cylsize has
   to be adjusted.
*/
	/*
	 * if the output file system is less than 2 cylinders, change cylsize
	 * so that file system is two cylinders 
	 */
	if (2*cylsize > super1.s_fsize)
			cylsize = super1.s_fsize / 2 ;
#endif
	/*
...
getimem()
{
	...
	avail = ((unsigned)ulimit(3,0L) - (unsigned)sbrk(0))/BSIZE;
	...
	if ( size < MIBLOCKS )
		err("Can't get inode buffer space");
	ibuf = (struct dinode *) space;
	niblks = size;
	nbufi = niblks * INOPB;
	printf("Got %u bytes for inodes (that's %u inodes)\n",
			size*BSIZE, nbufi);
#endif
}
