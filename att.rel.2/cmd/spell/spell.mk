...
# Modified: 06/11/85 by Zev Berger
# 	to build 'compress' for target 'all'.
...
compile all: spell spellin spellprog hashmake hashcheck compress nmake
	
nmake:
	if [ ! -d $(ROOT)/local/spell ] ; then mkdir $(ROOT)/local/spell; fi
	cp spell.nmk $(ROOT)/local/spell/spell.mk
	cp american british extra list local stop $(ROOT)/local/spell
	(cd $(ROOT) ; find local/spell usr/lib/spell usr/bin/spell -print \
		| cpio -oc > local/spell.cpio )
...
spellprog: spelldir spellprog.c hash.c hashlook.c huff.c malloc.c
	-if iAPX286;then \
	# malloc module does not handle requests larger than 32k	\
	$(CC) $(IFLAG) ${SMFLAG} ${FFLAG} -s -O spellprog.c hash.c hashlook.c huff.c -o spellprog ;	\
	else	\
	cc $(IFLAG)  ${FFLAG} -s -O spellprog.c hash.c hashlook.c huff.c malloc.c -o spellprog ;	\
	fi
	$(INS) $(PINSDIR) spellprog

spellin1: spelldir spellin.c huff.c
	cc $(IFLAG)  ${FFLAG} -O -s spellin.c huff.c -o spellin1
...
hashmk1: spelldir hashmake.c hash.c
	cc $(IFLAG) ${FFLAG} -O -s hashmake.c hash.c -o hashmk1
