...
#if iAPX286 && (FsTYPE == 2)
	if (nb & 1)	{
	  printf("Warning : odd number of physical blocks requested\n");
	  printf("          (rounding down to logical block boundary)\n");
			}
#endif
		if(!nb) {
			nb = n / (FSBSIZE/SECSIZE);
			n = nb/(NBINODE*4);
		} else {
			nb /= (FSBSIZE/SECSIZE);
			n /= NBINODE;
...
f2:
	nb = getnum();
	filsys->s_fsize = nb / (FSBSIZE/SECSIZE);
	n = getnum();
	n /= NBINODE;
	filsys->s_isize = n + 2;
...
f3:
	...
	printf("bytes per physical block = %d\n", SECSIZE);
