...
#if iAPX286
char Model = 's' ; /* used to select model */
#endif
...
#if iAPX286
		/*
		 * because of model dependencies the compiler produces
		 * conversion nodes for ptr = long and ptr = int
		 * in small and large model these need to have different
		 * affects
		 */

		if( SZLONG > SZINT && aflag
		   && ( tl == LONG || tl == ULONG )
		   && !( (SZPOINT == SZLONG) && (ISPTR(t)) )
...
#if iAPX286
	while((i=getopt(argc,argv,"abchnpuvxM:H:LT:X:")) != EOF)
#else
...
# if iAPX286
		/*
		 *	model dependant pointer sizes
		 */

		switch( Model ) {
			default:
				fprintf(stderr,"lint: unrecognised model - small assumed\n");
