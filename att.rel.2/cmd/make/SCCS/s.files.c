h07895
s 00001/00001/00912
d D 1.3 87/08/12 04:01:03 root 3 2
c changed DEPPATH to VPATH
e
s 00019/00002/00894
d D 1.2 87/08/12 03:08:45 root 2 1
c added DEPPATH
e
s 00896/00000/00000
d D 1.1 87/08/12 01:34:18 root 1 0
c date and time created 87/08/12 01:34:18 by root
e
u
U
t
...
D 3
	else if (dirpath || *(dirpath = varptr ("DEPPATH")->varval)) {
E 3
I 3
	else if (dirpath || *(dirpath = varptr ("VPATH")->varval)) {
E 3
	    while (*dirpath && *dirpath == ':')	/* skip over ':' */
		dirpath++;
	    if (*dirpath) {			/* if there's another dir */
		dirname = temp;			/* copy name to temp */
		while (*dirpath && *dirpath != ':')
		    *dirname++ = *dirpath++;
		*dirname = '\0';
		dirname = temp;
		goto nextdir;			/* and try search some more */
	    }
	}
