h49565
s 00032/00022/00956
d D 1.2 87/08/28 14:45:06 source 2 1
c Added "tar:" to error messages; fixed call to wait() to check return
c value; fixed abort when directory exists.
e
s 00978/00000/00000
d D 1.1 87/08/28 14:37:42 source 1 0
c date and time created 87/08/28 14:37:42 by source
e
u
U
t
...
time_t	time();	  /* this line added since time_t and int are different sizes */
char	*sprintf();
char	*strcat();
daddr_t	bsrch();
#define TBLOCK	512
...
E 2
I 2
		fprintf(stderr, "tar: directory checksum error: %s\n",
			dblock.dbuf.name);
E 2
		done(2);
	}
...
E 2
I 2
		if (checkdir(dblock.dbuf.name)) { /* directory indicator only */
		    if (vflag)
...
I 2
				...
				if (wtstat == -1)
				    ...
				    fprintf(stderr,"tar: mkdir %s error %x\n",
					name, i);
E 2
				if (!oflag)
