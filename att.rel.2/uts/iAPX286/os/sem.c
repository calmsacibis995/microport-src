...
semctl()
{
	...
	case IPC_SET:
		if(u.u_uid != sp->sem_perm.uid && u.u_uid != sp->sem_perm.cuid
			 && !suser())
			return;
		if(copyin(uap->arg.buf, &semtmp.ds, sizeof(semtmp.ds))) {
			u.u_error = EFAULT;
			return;
		}
	...
	case IPC_STAT:
		if(ipcaccess(&sp->sem_perm, SEM_R))
			return;
		if(copyout(sp, uap->arg.buf, sizeof(*sp))) {
			u.u_error = EFAULT;
			return;
		}
		return;
	...
	case GETALL:
		if(ipcaccess(&sp->sem_perm, SEM_R))
			return;
		u.u_base = (caddr_t)uap->arg.array;
		u.u_offset = 0;
		u.u_segflg = 0;
		for(i = sp->sem_nsems, p = sp->sem_base;i--;p++) {
	...
	case SETVAL:
		...
		if((unsigned)uap->arg.val > seminfo.semvmx) {
			u.u_error = ERANGE;
			return;
		}
		if((p = sp->sem_base + uap->semnum)->semval = uap->arg.val) {
			if(p->semncnt) {
				p->semncnt = 0;
				wakeup(&p->semncnt);
			}
...
seminit()
{
	...
	for (i = 0; i < seminfo.semmnu - 1; i++) {
		semfup->un_np = (struct sem_undo *)((caddr_t)semfup+seminfo.semusz);
		semfup = semfup->un_np;
	}
	semfup->un_np = NULL;
	semfup = semu;
