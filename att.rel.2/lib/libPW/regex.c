...
regex(addrc,addrl,a1,a2,a3,a4,a5) char *addrc, *addrl, *a1, *a2, *a3, *a4, *a5;
{
	...
	__cflg = 0;
	cur = (char *)__execute(addrc,addrl);
	adx = (pointer *)&a1;
	for(in=0;in<NBRA;in++) {
...
char *addrc,*addrl;
{
	...
	if (*p2==CIRCFL) {
		__loc1 = p1;
		return (i=__advance(p1,++p2))?i:(pointer)addrl;
	}
...
pointer alp, aep;
{
	...
	case PGRP|A512:
	case PGRP|A768:
		if (!(lp=(char *)__advance(lp,ep+1))) return((pointer)0);
	case SGRP|A768:
	case SGRP|A512:
	case SGRP|A256:
	case SGRP:
	...
	case CCL|PLUS:
	case NCCL|PLUS:
		if (!__cclass(ep,*lp++,ep[-1]==(CCL|PLUS))) return((pointer)0);
	case CCL|STAR:
	case NCCL|STAR:
		curlp = lp;
