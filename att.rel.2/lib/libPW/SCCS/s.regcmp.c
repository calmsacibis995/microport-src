h21592
s 00243/00000/00000
d D 1.1 87/04/10 10:35:31 root 1 0
c date and time created 87/04/10 10:35:31 by root
e
u
U
t
			...
			*ep++ = 0;
			...
			do {
				...
				if ((c=='-') && (cclcnt>1) && (*sp!=']')) {
					...
					*ep++ = ep[-1];
#else
					/* this line of C is non-portable */
					/* and had to be split to get correct */
					/* evaluation order on iAPX286 */
					*ep = ep[-1];
					ep++;
#endif
					ep[-2] = MINUS;
