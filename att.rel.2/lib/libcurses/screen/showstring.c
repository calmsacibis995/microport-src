...
#ifdef DEBUG
#if iAPX286 & LARGE_M
	if(outf) fprintf(outf, "_showstring((%d,%d) %ld:'", sline, scol, last-first+1);
#else
...
#ifndef		NONSTANDARD
#if iAPX286 & LARGE_M
		fprintf(stderr, "Bad call to _showstring, first %lx, last %lx,\
...
#ifdef FULLDEBUG
#if iAPX286 & LARGE_M
	if(outf) fprintf(outf, "p %lx '%c' %o, lastc %x %o, oldp %lx %o, hl %d\n", p, p[-1], p[-1], lastc, lastc[-1], oldp, oldp ? oldp[-1] : 0, hl);
#else
...
#ifdef FULLDEBUG
#if iAPX286 & LARGE_M
	if(outf) fprintf(outf, "p %lx '%c%c%c%c', oldp %lx '%c%c%c%c', hl %d\n",
#else
...
#ifdef DEBUG
#if iAPX286 & LARGE_M
	if(outf) fprintf(outf, "oldlp %lx, oldline %lx, oldlen %d 0x%x\n", oldlp, oldline, oldlen, oldlen);
#else
...
#ifdef DEBUG
#if iAPX286 & LARGE_M
		if(outf) fprintf(outf, "p %lx *p %o, pc %lx *pc %o, oldp %lx, *oldp %o, lcol %d, lrow %d, oldc %o\n", p, *p, pc, *pc, oldp, *oldp, lcol, lrow, oldc);
#else
