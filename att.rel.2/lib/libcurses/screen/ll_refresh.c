...
int	use_idl;
{
...
#ifdef DEBUG
#if iAPX286
	if (outf && SP->std_body[SP->virt_y+1] && SP->std_body[SP->virt_y])
#else
	if (outf)
#endif
		fprintf(outf, "virt cursor at y=%d, x=%d, length %d, length-1 %d\n",
	...
	{
#if iAPX286 & LARGE_M
		if(outf) fprintf(outf, "%8lx: ", SP->cur_body[i]);
#else
	...
	{
#if iAPX286 & LARGE_M
		if(outf) fprintf(outf, "%8lx: ", SP->std_body[i]);
#else
