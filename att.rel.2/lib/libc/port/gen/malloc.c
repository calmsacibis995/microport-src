...
unsigned nbytes;
{
...
#if iAPX286 && (LARGE_M || HUGE_M || COMPACT_M)
	/* cannot malloc more than 1 segment plus room for 3 WORDS */
	temp = ((nw+WORD+WORD+BLOCK/WORD-1)/(BLOCK/WORD));
	if( ( (unsigned int) (temp*BLOCK)) < nbytes)
		return ( NULL ) ;
#endif
	...
	for(p=allocp; ; ) {
		...
		sav_temp = temp;
#if iAPX286 && (LARGE_M || HUGE_M || COMPACT_M)
		/* use brk to get space */
		
		if(brk( (char *) ( (long)q + (long)temp ) )  == -1 )
			return NULL ;
#else
		if (temp > MAXALLOC)  {
